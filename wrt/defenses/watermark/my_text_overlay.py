...
class Text_overlay(Watermark):
    ...
    def keygen(wm_loader: data.DataLoader,
        ...
        for data in self.wm_set:
            x, y = data
            x_wm.append(x)
            y_wm.append(torch.tensor(y))

        x_wm = torch.stack(x_wm)
        y_wm = torch.stack(y_wm)
...
@mlconfig.register
def text_overlay(classifier, **kwargs):
    return Text_overlay(classifier=classifier, **kwargs)


@mlconfig.register
def text_overlay_embed(defense: Text_overlay, **kwargs):
    return defense.embed(**kwargs), defense


@mlconfig.register
def text_overlay_keygen(defense: Text_overlay,
                  train_loader,
                  wm_loader: data.DataLoader,
                  num_classes: int,
                  keylengths: List[int],
